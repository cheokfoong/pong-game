...
function drawRectsObservable() {
    const svg = document.getElementById("drawRects"), mousemove = Observable.fromEvent(svg, 'mousemove'), mouseup = Observable.fromEvent(svg, 'mouseup'), svgRect = svg.getBoundingClientRect(), o = Observable
        .fromEvent(document, "mousedown")
        .map(({ clientX, clientY }) => ({ r: new Elem(svg, 'rect')
            .attr('x', clientX - svgRect.left).attr('y', clientY - svgRect.top)
            .attr('width', 5).attr('height', 5)
            .attr('fill', '#95B3D7'), x0: clientX - svgRect.left, y0: clientY - svgRect.top }))
        .flatMap(({ r, x0, y0 }) => mousemove
        .takeUntil(mouseup)
        .map(({ clientX, clientY }) => ({ x1: clientX - svgRect.left, y1: clientY - svgRect.top }))
        .map(({ x1, y1 }) => ({ left: Math.min(x0, x1), top: Math.min(y0, y1), width: Math.abs(x0 - x1), height: Math.abs(y0 - y1), r })))
        .subscribe(({ left, top, width, height, r }) => r
        .attr('x', String(left)).attr('y', String(top))
        .attr('width', String(width)).attr('height', String(height)));
}
function drawAndDragRectsObservable() {
    const svg = document.getElementById("drawAndDragRects"), mousemove = Observable.fromEvent(svg, 'mousemove'), mouseup = Observable.fromEvent(svg, 'mouseup'), rect = new Elem(svg, 'rect')
        .attr('x', 0).attr('y', 0)
    ...
    rect.observe('mousedown')
        ...
        .attr('width', String(width)).attr('height', String(height))
        .observe('mousedown')
        .map(({ clientX, clientY }) => ({ xOffset: Number(r.attr('x')) - clientX,
        yOffset: Number(r.attr('y')) - clientY }))
        .flatMap(({ xOffset, yOffset }) => mousemove
        .takeUntil(mouseup)
        .map(({ clientX, clientY }) => ({ x: clientX + xOffset, y: clientY + yOffset })))
        .subscribe(({ x, y }) => r.attr('x', x)
        .attr('y', y)));
}
